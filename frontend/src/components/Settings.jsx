...
function RiskTargetSection() {
  ...
  const [display, setDisplay] = useState('$') // local-only: $ or %
  ...
  // Inline computed label shown to the right of the input row
  ...
  return (
    <div className={CARD}>
      <div className="px-5 py-4 border-b border-border flex items-start justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Risk &amp; Targets</p>
          <p className="text-xs text-muted-foreground mt-1">Set your daily risk limit and profit target. Hitting either flags the day as overtrading.</p>
        </div>
        {/* Display toggle — local only, just for the hints below */}
        <div className="flex border border-border text-[10px] font-bold uppercase tracking-wider shrink-0 self-center">
          {['$', '%'].map(m => (
            <button
              key={m}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={e => {
                ...
                setCfg(c => cfg.dailyRiskMode === 'pct'
                  ? { ...c, dailyRiskPct: v }
                  : { ...c, dailyRiskFixed: v })
              }}
            />
            ...
            {riskInline && (
              <span className="font-mono text-sm font-semibold text-rose-400 shrink-0 min-w-[52px] text-right">{riskInline}</span>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={e => {
                const v = parseFloat(e.target.value) || 0
                setCfg(c => cfg.dailyTargetMode === 'pct'
                  ? { ...c, dailyTargetPct: v }
                  : { ...c, dailyTargetFixed: v })
              }}
            />
            <ModeToggle value={cfg.dailyTargetMode} onChange={v => setCfg(c => ({ ...c, dailyTargetMode: v }))} />
            {targetInline && (
              <span className="font-mono text-sm font-semibold text-emerald-400 shrink-0 min-w-[52px] text-right">{targetInline}</span>
            )}
          </div>
        </div>
  ...
  return (
    <div className={CARD}>
      <div className="px-5 py-4 border-b border-border">
        <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">Accounts</p>
        <p className="text-xs text-muted-foreground mt-1">Create separate accounts (e.g. Swing, 0DTE) to track trades independently.</p>
      </div>
...
export default function Settings() {
  ...
  const [recomputeStatus, setRecomputeStatus] = useState(null) // null | 'loading' | 'done' | 'error'
  ...
  const summary = `Top ${(cfg.marginTop * 100).toFixed(0)}% · Bottom ${(cfg.marginBottom * 100).toFixed(0)}% · Right ${cfg.rightBars} bars`

  return (
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-lg space-y-3">

        {/* ── Chart Defaults (collapsible) ─────────────────────────────────── */}
        <div className={CARD}>
          <button
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`text-muted-foreground transition-transform shrink-0 ml-4 ${chartOpen ? 'rotate-180' : ''}`}
            />
          </button>
            ...
            <div className="px-5 pb-5 border-t border-border">
              <div className="space-y-4 mt-4">
                <NumField
                  label="Top margin"
                  value={cfg.marginTop}
                  min={0.01} max={0.48} step={0.01}
                  unit={`${(cfg.marginTop * 100).toFixed(0)}%`}
                  onChange={set('marginTop')}
                />
                <NumField
                  label="Bottom margin"
                  value={cfg.marginBottom}
                  min={0.01} max={0.48} step={0.01}
                  unit={`${(cfg.marginBottom * 100).toFixed(0)}%`}
                  onChange={set('marginBottom')}
                />
                <NumField
                  label="Right-side empty bars"
                  value={cfg.rightBars}
                  min={0} max={50} step={1}
                  unit="bars"
                  onChange={set('rightBars')}
                />
              </div>
              ...
              <div className="flex gap-2 mt-5">
                ...
                <button
                  onClick={handleReset}
                  className="px-4 py-2 text-xs font-bold uppercase tracking-wider border border-border text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"
                >
                  Reset
                </button>
              </div>
        ...
        <div className={CARD}>
          <div className="px-5 py-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Used for date/time defaults when logging trades and for chart time labels.
            </p>
            <select
              value={tz}
        ...
        <div className={CARD}>
          <div className="px-5 py-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Used to calculate post-exit MFE — how much a trade moved after you closed it.
            </p>
            <div className="flex items-center gap-3">
              <div className="flex-1">
                ...
                <input
                  type="time"
                  value={appCfg.tradingStart}
                  onChange={e => setAppCfg(c => ({ ...c, tradingStart: e.target.value }))}
                  className={INPUT}
                />
              </div>
              <div className="flex-1">
                ...
                <input
                  type="time"
                  value={appCfg.tradingEnd}
                  onChange={e => setAppCfg(c => ({ ...c, tradingEnd: e.target.value }))}
                  className={INPUT}
                />
              </div>
            </div>
            <button
              ...
              style={{
                background: hoursSaved ? '#16a34a22' : 'hsl(var(--primary)/0.15)',
                color:      hoursSaved ? '#4ade80'   : 'hsl(var(--primary))',
                border:     `1px solid ${hoursSaved ? '#16a34a44' : 'hsl(var(--primary)/0.3)'}`,
              }}
            >
        ...
        <div className={CARD}>
          <div className="px-5 py-4 border-b border-border">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">MAE / MFE</p>
            <p className="text-xs text-muted-foreground mt-1">Thresholds for good-setup detection and edge calculations.</p>
          </div>
          <div className="px-5 py-4 space-y-4">
            <NumField
            ...
            <button
              onClick={handleMaeSave}
              className="w-full py-2 text-xs font-bold uppercase tracking-wider transition-colors"
              style={{
                background: maeSaved ? '#16a34a22' : 'hsl(var(--primary)/0.15)',
                color:      maeSaved ? '#4ade80'   : 'hsl(var(--primary))',
                border:     `1px solid ${maeSaved ? '#16a34a44' : 'hsl(var(--primary)/0.3)'}`,
              }}
            >
              {maeSaved ? '✓ Saved' : 'Save'}
            </button>
          ...
          {recomputeStatus && recomputeStatus !== 'loading' && (
            <div
              className={`mx-5 mb-4 px-4 py-2.5 flex items-center gap-2 text-xs ${
        ...
        <div className={CARD}>
          <div className="px-5 py-4">
            ...
            <p className="text-xs text-muted-foreground">
              Export all trades and exits as JSON, or import a previously exported file.
            </p>
          </div>
          ...
          <div className="border-t border-border px-5 py-4 flex gap-3 flex-wrap">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-wider border border-border text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"
            >
              <Download size={12} />
              Export JSON
            </button>
